      ...
      <Container contained={true}>
        ...
        >
          {features.map((feature, key) => (
            <figure className={`${key < features.length - 1 ? "mb-40 md:mb-0" : ""}`} key={key}>
              <div className={`text-center ${styles.featureIllustration}`}>
                ...
                <ReactSVG src={feature.illustration.url} />
              </div>
              <figcaption className="text-left mt-30 md:mt-50">{RichText.render(feature.text)}</figcaption>
            </figure>
          ))}
        </animated.div>
        <animated.div
          className="flex justify-center mt-30 mb-40 md:mt-0"
          style={useSpring({
