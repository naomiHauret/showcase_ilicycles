...
const TransportCapacity = memo((props) => {
  ...
  return (
    <section ref={ref} className={`relative mt-80 ${styles.transportCapacity} flex flex-col-reverse md:block`}>
      <animated.div
        style={useSpring({
      ...
      <Container contained={true} staticStyles={`md:grid md:gap-30 md:grid-col-12 ${styles.contentWrapper}`}>
        <div className={`${styles.content} md:text-right`}>
          ...
          >
            ...
            <ul
              className={`${styles.featuresGrid} grid m-0 mt-50 md:mt-40 p-0 list-none md:text-15 text-black-100 font-emphasis tracking-045 md:tracking-052`}
            >
              {features.map((feature, key) => (
                <li className="items-center m-0 p-0 flex" key={key}>
                  <ReactSVG src={feature.icon.url} />
                  <span className="block ml-10 md:ml-15 text-left">{feature.label}</span>
                </li>
              ))}
            </ul>
          </animated.div>
