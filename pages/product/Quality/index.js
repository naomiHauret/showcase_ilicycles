...
const Quality = memo(
  (props) => {
    ...
    return <section ref={ref}>
      <Container contained={true} staticStyles={`flex flex-col-reverse md:grid md:gap-30 md:grid-col-12 ${styles.grid}`}>
      ...
      <MediaQuery query={`(min-width: ${pxTo(ds.get('breakpoints.md'), baseFontSize, "rem")})`}>
        ...
        <div className={styles.gridCol2}>
          <animated.h1 className="text-center md:text-right text-blue-100" style={
            useSpring({
              to: { opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(20px)' }, from: { opacity: 0, transform: 'translateY(20px)' },
              delay: 150,
            })
          }>{title}</animated.h1>
          <animated.p className="text-center md:text-right mt-15 mb-10 md:mt-30 md:mb-10 text-blue-100 text-base" style={
            useSpring({
              to: { opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(15px)' }, from: { opacity: 0, transform: 'translateY(15px)' },
              delay: 250,
            })
          }>{subtitle}</animated.p>
          <animated.div className="text-center mb-30 md:mb-0 md:text-right font-emphasis text-blue-100 text-base font-600" style={
            useSpring({
          ...
          <animated.div className="text-left md:text-right md:mt-30" style={
            useSpring({
              to: { opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(10px)' }, from: { opacity: 0, transform: 'translateY(10px)' },
              delay: 450,
            })
      ...
      <MediaQuery query={`(max-width: 990px)`}>
        ...
        <Container contained={true}>
          <animated.div className="text-left" style={
            useSpring({
              to: { opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(10px)' }, from: { opacity: 0, transform: 'translateY(10px)' },
              delay: 450,
            })
